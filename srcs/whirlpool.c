...
static uint8_t *getConstantRoundMatrix(int r, uint8_t *cr)
{
	ft_memset(cr, 0, 64);
...
static void whirlpoolRound(uint8_t *matrix, uint8_t *key)
{
	for (int i = 0; i <= 7; i++)
	{
		for (int j = 0; j <= 7; j++)
...
char *whirlpool(char *str)
{
	...
	for (size_t chunk = 0; chunk < pad_len; chunk += 64)
	{
		...
		{
			uint8_t cr[64];
			getConstantRoundMatrix(r, cr);
			whirlpoolRound(Kr, cr);
			whirlpoolRound(w, Kr);
		}
